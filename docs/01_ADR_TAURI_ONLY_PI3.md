...
- Новые фичи и экраны реализуются в web-слое (`src/**`) и должны работать в `dev:mock/dev:live`.
- Проверка Tauri обязательна при изменениях в `src-tauri/**` и перед релизными поставками.
